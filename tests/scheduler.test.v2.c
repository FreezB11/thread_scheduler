...
void worker1(void *arg){
    for(int i = 0; i < 5; i++){
        printf("worker1: %d\n",i);
        /*
            the point is that since the timer was of 10ms and this is processing 
            everything within that span so we add the usleep();
        */
        usleep(5000); // we add this to see the preemptive 
        // uthr_yield();
    }
}
